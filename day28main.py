from tkinter import *
import math
# ---------------------------- CONSTANTS ------------------------------- #
PINK = "#e2979c"
RED = "#e7305b"
GREEN = "#9bdeac"
YELLOW = "#f7f5dd"
FONT_NAME = "Courier"
WORK_MIN = 25
SHORT_BREAK_MIN = 5
LONG_BREAK_MIN = 20
reps=0
timer=None
mark=""
# ---------------------------- TIMER RESET ------------------------------- # 

# ---------------------------- TIMER MECHANISM ------------------------------- # 
def start_button_clicked():
    global reps
    reps+=1
    work_sec=WORK_MIN*60
    short_break_sec=SHORT_BREAK_MIN*60
    long_break_sec=LONG_BREAK_MIN*60

    if reps%8==0:
        count_down(long_break_sec)
        my_label_timer.config(text="Break",fg=RED)

    elif reps%2==0:
        count_down(short_break_sec)
        my_label_timer.config(text="Break",fg=PINK)
    else:
        count_down(work_sec)
        my_label_timer.config(text="Work",fg=GREEN)


# ---------------------------- COUNTDOWN MECHANISM ------------------------------- # 
def count_down(count):

    count_min=math.floor(count/60)
    count_sec=count%60
    if count_sec <10:
        count_sec=f"0{count_sec}"


    canvas.itemconfig(timer_text,text=f"{count_min}:{count_sec}")
    timer_text
    if count>0:
        global timer
        timer =  window.after(1000,count_down,count-1)
    else:
        start_button_clicked()
        global mark
        mark=""
        work_sessions=math.floor(reps/2)
        for n in range(work_sessions):
            mark +="✔"
    my_label_check_mark.config(text=mark)


def reset_timer():
    window.after_cancel(timer)
    canvas.itemconfig(timer_text,text="00:00")
    my_label_timer.config(text="Timer")
    my_label_check_mark.config(text="")

# ---------------------------- UI SETUP ------------------------------- #

window=Tk()
window.title("POMODORO")
window.config(padx=100,pady=50)

window.config(background=YELLOW)



canvas=Canvas(width=200,height=224,background=YELLOW)


#IF BORDER COMING USE highlightthickness=0 in canvas 

tomato_img=PhotoImage(file="tomato.png")

canvas.create_image(102,112,image=tomato_img)
timer_text=canvas.create_text(102,130,text="00:00",fill="white",font=(FONT_NAME,35,"bold"))
canvas.grid(row=1,column=1)


my_label_timer=Label(text="Timer",font=(FONT_NAME,35,"bold"),fg=GREEN,bg=YELLOW)

my_label_timer.grid(row=0,column=1)


start_button=Button(text="Start",command=start_button_clicked)
start_button.grid(row=2,column=0)


reset_button=Button(text="Reset",command=reset_timer)
reset_button.grid(row=2,column=3)

my_label_check_mark=Label(font=(FONT_NAME,15,"bold"),bg=YELLOW,fg=GREEN)
my_label_check_mark.grid(row=3,column=1)




window.mainloop()
